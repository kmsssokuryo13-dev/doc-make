...
export const DocRow = ({ name, count, isRequired, sources, onChange, min = 0, max = Infinity }) => (
  <div className={`p-4 rounded-xl border-2 bg-white transition-all flex items-center justify-between ${count > 0 ? 'border-blue-400 shadow-sm' : 'border-slate-100 opacity-60'}`}>
    ...
    <div className="flex items-center gap-4 shrink-0 ml-4 font-bold">
      ...
      <span className="w-8 text-center font-mono font-bold text-sm">{count}</span>
      <button
        type="button"
        onClick={() => onChange(1)}
        disabled={count >= max}
        className="p-1.5 rounded bg-slate-100 hover:bg-blue-600 hover:text-white transition-all active:scale-95 text-blue-600 font-black disabled:cursor-not-allowed disabled:hover:bg-slate-100 disabled:hover:text-blue-600 disabled:opacity-30"
        aria-label={`${name}を1通増やす`}
        title={count >= max ? `1申請あたり最大${max}通です` : undefined}
      ><Plus size={16}/></button>
    </div>
  </div>
);
