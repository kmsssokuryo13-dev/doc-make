...
export const Editor = ({ sites, setSites, activeSiteId, setActiveSiteId, contractors, setContractors, scriveners, setScriveners }) => {
  ...
  const exportToJson = () => {
    const data = buildExportPayload({ activeSiteId, sites, contractors, scriveners });
  ...
  const importFromJson = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (parsed.scriveners) setScriveners(parsed.scriveners);
          ...
          {(sites || []).map(site => (
            <div key={site.id} onClick={() => setActiveSiteId(site.id)} className={`group px-4 py-4 cursor-pointer border-b border-slate-700/50 flex flex-col gap-1 transition-all relative ${activeSiteId === site.id ? 'bg-blue-600/20 border-l-4 border-l-blue-500' : 'hover:bg-slate-700/50 border-l-4 border-l-transparent'}`}>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                ...
                <button onClick={(e) => { e.stopPropagation(); setSiteToDelete(site); setIsDeleteModalOpen(true); }} title="この現場を削除" className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 transition-opacity shrink-0 -my-0.5 p-0.5"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
                ...
                <>
                  <div className="bg-white border-b border-gray-200 shadow-sm z-10 px-4 py-1 flex items-center gap-1 overflow-x-auto no-scrollbar font-bold">
                    ...
                    <div className="ml-auto flex items-center gap-2 shrink-0">
                      <button onClick={() => navigate(`/docs?siteId=${activeSite.id}`)} title="閲覧中の現場の書類を作成" className="flex items-center gap-1 py-1 px-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-[10px] font-bold shadow active:scale-95 transition-all"><FileText size={12} /> 書類作成</button>
